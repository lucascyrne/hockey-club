import { TABLE_BORDER_HEIGHT, TABLE_DEPTH, TABLE_SURFACE_THICKNESS, TABLE_WIDTH } from './table'

/** Corte diagonal nos 4 cantos (~8 cm) — evita bolso 90°. */
...
export type CornerLayout = {
  ...
  /** Rapier half-extents: [alongDiagonal, heightY, thicknessOutward] */
...
function rotYForCorner(signX: 1 | -1, signZ: 1 | -1): number {
  return ROT_Y[`${signX},${signZ}`] ?? 0
}

/** Layout partilhado física + visual do chanfro diagonal. */
export function getCornerChamferLayout(
  signX: 1 | -1,
  signZ: 1 | -1,
): CornerLayout {
  ...
  return {
    ...
    rotation: [0, rotYForCorner(signX, signZ), 0],
    args: [halfAlong, WALL_HALF_HEIGHT, WALL_HALF_THICK],
  }
}

/** Limiar diagonal interior: signX*x + signZ*z <= esta constante (lado jogável). */
export function cornerDiagonalMaxSum(): number {
  return (hw - chamfer) + (hd - chamfer)
}

export function cornerSignsFromPosition(x: number, z: number): {
  signX: 1 | -1
  signZ: 1 | -1
} | null {
...
/** Disco no triângulo morto do canto (fora do campo útil). */
...
/** Projeção do ponto para o segmento da chanfra + normal interior (para o campo). */
